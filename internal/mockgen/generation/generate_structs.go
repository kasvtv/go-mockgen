...
func generateMockStruct(iface *wrappedInterface, outputImportPath string) jen.Code {
	...
	for _, method := range iface.wrappedMethods {
		...
		mockFuncField := compose(jen.Id(mockFuncFieldName), addTypes(jen.Op("*").Id(mockFuncStructName), iface.TypeParams, false))
		structFields = append(structFields, addComment(mockFuncField, 2, commentText))
	}
